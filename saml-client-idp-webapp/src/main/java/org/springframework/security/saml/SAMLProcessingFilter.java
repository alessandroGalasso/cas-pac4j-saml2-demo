...
public class SAMLProcessingFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
    	...
    	System.out.println("----> SAMLProcessingFilter attemptAuthentication");
        ...
        try {
            ...
            */
            ...
            // getPrincipal getClass() class org.opensaml.saml2.core.impl.NameIDImpl
            //System.out.println("authentication.getPrincipal getClass() "+authentication.getPrincipal().getClass());
            
            
            
          NameIDImpl nameIDImpl = (NameIDImpl) authentication.getPrincipal();
          //<saml2:Subject>  <saml2:NameID   >_b524a7d9b477c3be346c851933a50f45</saml2:NameID>
          System.out.println("nameIDImpl "+ nameIDImpl.getValue() );
         
          //<saml2:Subject> <saml2:NameID 
          //Format="urn:oasis:names:tc:SAML:2.0:nameid-format:transient" 
          //NameQualifier="https://idp.alessandro.it/idp/shibboleth">  
            NamedNodeMap nameIDImplattributes = nameIDImpl.getDOM().getAttributes();
            System.out.println(" nameIDImplattributes.getLength() "+nameIDImplattributes.getLength() );
             for(int i=0;i<nameIDImplattributes.getLength();i++){
            	Node node = nameIDImplattributes.item(i);
            	System.out.println(" node.getNodeValue() " +node.getNodeValue());
            }
            
            //getCredentials getClass() class org.springframework.security.saml.SAMLCredential
            SAMLCredential sAMLCredential =  (SAMLCredential) authentication.getCredentials();
            
            System.out.println("--->SAMLProcessingFilter sAMLCredential.getNameID().getValue() "+sAMLCredential.getNameID().getValue());
            ...
            List<XMLObject> attributeValues = attribute.getAttributeValues();
         	  ...
         	  for(int j=0;j<attributeValues.size();j++){
          		  XMLObject attributeValue = attributeValues.get(j);
            	  System.out.println("--->attributeValue "+attributeValue.getClass());
            	  try{
          	      XSStringImpl xSStringImpl = (XSStringImpl) attributeValue;
          		  System.out.println( "nome attributo  "+  xSStringImpl.getElementQName().toString() );
          		  System.out.println( "valore attributo   "+  xSStringImpl.getValue() );
            	  }catch(Exception e){}
